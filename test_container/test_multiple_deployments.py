from uuid import uuid4 as uuid
import logging as log
import requests
import time
import sys
import os

import helpful_test_functions as helpers

log.basicConfig(level=log.DEBUG, stream=sys.stdout)

test_url = next(helpers.generate_test_urls(n=1))

r = helpers.requests_log_wrapper(requests)

try:
    response = r.get(test_url)
    assert (
        500 >= response.status_code >= 400
    ), f"""
    Failed to check that running against an unknown test id would
    result in a 4xx response.

    url={test_url}

    {response.text}

    """

    for img in helpers.generate_test_images(10):
        body = {
            "domain": "my-domain.com",
            "build_id": uuid().hex,
            "run_id": uuid().hex,
            "image": img,
            "model_type": "default",
        }
        response = r.post(test_url, json=body)
        assert (
            200 == response.status_code
        ), f"""
        Failed to create a request 

        url={test_url}
        body={body}

        {response.text}
        """
        time.sleep(30)

        response = r.get(test_url + "?verbose=true")
        assert (
            200 == response.status_code
        ), f"""
        Failed to retrieve previously created configuration 

        url={test_url}

        {response.text}
        """
        data = response.json()
        assert all(
            [
                data[0]["build_id"] == body["build_id"],
                data[0]["run_id"] == body["run_id"],
            ]
        ), f"""
        Expected build id and run id to match between created deployment
        and retrived deployment:

        created_deployment:
        {body}

        retrieved_deployment:
        {data}
        """
finally:
    response = r.delete(test_url + "?clear_all_records=true")
    assert (
        200 == response.status_code
    ), f"""
     Failed to delete test record. Watch out we still have lingering shit!

     url={test_url}

     {response.text}
     """
